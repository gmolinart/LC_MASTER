...
def get_default_shader():
    ...
    default_shader = path_object.copy(scope='assets',
                                      type='lib',
                                      task='shd',
                                      user='publish',
                                      resolution='high',
    ...
    print('_______ASSIGNING SHADER FROM :')
    print(default_shader.path)

    if os.path.isfile(default_shader.path_root):

        path_object = default_shader.copy(major='003', minor='000')
    ...
    if not default_in_scene:
        lm.import_file_old(filepath=path_object.path_root, linked=False, type='GROUP')

    return (default_shader.version)
...
def updateShader(material):
    ...
    for node in nodes:
        ...
        if node.type == 'TEX_IMAGE':
            print('________{}______'.format(node.type))
            print(node.name)
            links.new(node_tree.nodes['DEFAULTSHADER'].inputs['Color'],
                      node.outputs[0])
...
def run():
    ...
    if len(failed_items):
        ...
        lm.confirm_prompt(message='Default shader assigned version:  {}'.format(shader))
